...
`warpctx current` naming a profile — and even `warpctx` reporting "Connected / healthy / 0% loss" — is **NOT** evidence the tunnel reaches the cluster. It has reported healthy for hours while private routes were dead (`kubectl` → "network is unreachable", `internal.surge.io` resolving to nothing), then recovered on its own. After your claim lands and before any dev or prod mutation, run **`coord probe warp --profile Dev`** (or `--profile Prod`) — which runs a real in-cluster call that traverses the tunnel and returns exit `11` if it fails. If it fails while WARP claims healthy, **the tunnel is the prime suspect**.

**The probe targets the profile's own kube context, never the default.** A bare `kubectl get ns` uses the kubeconfig default context, which on this machine is `prod` — so probing a Dev lease that way would reachability-test PROD, fail, and falsely report Dev as down. `coord probe` resolves the profile to its context via the registry's `[resource.warp.contexts]` map (`Dev -> surge-dev`, `Prod -> prod`) and runs `kubectl --context <mapped> get ns`, printing the context it tested so the verdict is self-describing. If you omit `--profile` it uses the held lease's recorded profile; if neither resolves, **the probe REFUSES with exit `12`** rather than silently testing the default context (a probe that can pass/fail against the wrong cluster is worse than none). `--cmd "..."` still overrides the whole command verbatim when you need it.
