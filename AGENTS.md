...
**fff MCP roots — pass `base_path` or you search the wrong tree.** fff MCP is a shared singleton with ONE global default root: the *personal* vault (`abhi.easygo.io`). Every fff tool takes an optional `base_path`; omitting it searches that default, **not** your working tree — so a canon/engineering search silently returns nothing and you wrongly conclude "not found" or fall back to `rg`. Both surfaces are told up front: the `SessionStart` preload lists every configured root and resolves the session cwd → the exact `base_path`, and the `SubagentStart` digest carries the same cwd hint. Both read fff's own `config.toml` through `hooks/lib/fff_roots.py`, so they never disagree. Silence means the default root already covers your cwd; if the hint is missing anyway, call `mcp__fff__list_roots` and pass the `base_path` of the root whose path contains it. Most-specific root wins — a nested corpus repo (`surge.easygo.io`, `surge.app`) beats the umbrella `surge.workspace`, whose `.gitignore` prunes those nested repos anyway. See [[fff-default-root-personal-vault]].
...
**The same hard requirement applies to opaque identifiers** — in chat responses, vault notes, and source-repo artifacts (commits, PRs, docs, code comments) alike. Any short alphanumeric handle drawn from a plan/brainstorm/decision artifact or a project taxonomy is as uncorrelatable as an unexpanded acronym. This covers the full `ce-plan`/`ce-brainstorm` ID family — `U<n>` (implementation unit), `F<n>` (feature), `R<n>` (requirement), `AE<n>` (acceptance example), `KTD<n>` (key technical decision), `KD<n>` (key decision) — and project handles: `ADR-NNN`, `BC-N`, `SD-N` (shared-domain card), `CD-NN` (code-design card), and the like. On **first use in any document**, attach a slug or brief expansion in brackets — inline (`U4 (author base contracts)`, `ADR-082 (domain event transport)`, `KTD-3 (idempotent outbox writes)`), or, when inline would clutter dense prose, a small glossary/legend at the end mapping each handle to its title. Never the bare handle alone on first use. Bare handles are fine in later uses within the same document. If a document uses none, the rule is satisfied automatically.
